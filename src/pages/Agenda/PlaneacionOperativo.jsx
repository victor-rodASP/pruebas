...
const ClientNameInput = ({ value, idx, segmentName, updateVisit, clientsDatabase }) => {
    const [query, setQuery] = useState(value || '');
    const [showSuggestions, setShowSuggestions] = useState(false);
    const containerRef = useRef(null);

    const isCarteraSegment = segmentName.toLowerCase().includes('seguimiento') || 
                             segmentName.toLowerCase().includes('gestión') || 
                             segmentName.toLowerCase().includes('visita') || 
                             segmentName.toLowerCase().includes('cartera') ||
                             segmentName.toLowerCase().includes('compromiso');

    const suggestions = (clientsDatabase || []).filter(item => {
        if (query.length <= 1) return false;
        if (!item.name.toLowerCase().includes(query.toLowerCase())) return false;
        if (item.name === query.toUpperCase()) return false;

        if (isCarteraSegment && item.classification !== 'Cliente') return false;
        if (!isCarteraSegment && item.classification === 'Cliente') return false;
            ...
            {showSuggestions && suggestions.length > 0 && (
                <div className="absolute z-[100] top-full mt-1 w-full bg-white rounded-xl shadow-2xl border border-slate-100 py-1.5 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="px-3 py-1 text-[8px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50 mb-1">
                        {isCarteraSegment ? 'Cartera encontrada' : 'Contactos encontrados'}
                    </div>
                    {suggestions.map((item, sIdx) => (
                        ...
                        >
                            <div className="flex flex-col">
                                ...
                                <span className="text-[9px] font-medium text-slate-400">{item.streets || 'Sin dirección'} | {item.classification}</span>
                            </div>
...
const DesktopRowFull = ({ v, idx, segmentName, updateVisit, removeRow, isTimeAvailable, clientsDatabase }) => {
    const { catalogs } = useAgenda();
    const nameLower = segmentName.toLowerCase();
    
    const isClientOnlySegment = nameLower.includes('seguimiento') || nameLower.includes('cartera') || nameLower.includes('gestión') || nameLower.includes('visita') || nameLower.includes('compromiso');
    ...
    const renderSegmentDetails = () => {
        ...
        if (nameLower.includes('evaluación') || nameLower.includes('integración') || nameLower.includes('prospecto')) {
            return (
        ...
        if (nameLower.includes('compromiso')) {
            return (
                <div className="grid grid-cols-4 gap-4 bg-amber-50/30 p-4 rounded-xl border border-amber-100 mt-2 ml-14 shadow-sm">
                    <div>
                        <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 block pl-1">ID Crédito / Control</label>
                        <div className="input-cell font-mono-tech flex items-center px-4 h-[42px] bg-white border-slate-100 text-primary">
                            {v.idCredito || v.idControl || 'S/N'}
                        </div>
                    </div>
                    <div>
                        <label className="text-[8px] font-black text-amber-600 uppercase tracking-widest mb-1 block pl-1">Fecha Promesa</label>
                        <input 
                            type="date" 
                            value={v.fechaCompromiso || ''} 
                            onChange={e => updateVisit(segmentName, idx, 'fechaCompromiso', e.target.value)}
                            className="input-cell h-[42px] text-[11px] font-bold text-amber-700 uppercase border-amber-200 focus:border-amber-400 focus:ring-amber-400/20 bg-white"
                        />
                    </div>
                    <div>
                        <label className="text-[8px] font-black text-amber-600 uppercase tracking-widest mb-1 block pl-1">Monto a Pagar</label>
                        <div className="relative">
                            <input 
                                type="text"
                                value={v.isFocusedAmount ? (v.montoCompromiso || '') : formatCurrency(v.montoCompromiso)}
                                onFocus={() => updateVisit(segmentName, idx, 'isFocusedAmount', true)}
                                onBlur={() => updateVisit(segmentName, idx, 'isFocusedAmount', false)}
                                onChange={e => handleNumericChange(e.target.value, (val) => updateVisit(segmentName, idx, 'montoCompromiso', val))}
                                placeholder="$ 0.00"
                                className="input-cell h-[42px] font-bold text-amber-700 border-amber-200 focus:border-amber-400 focus:ring-amber-400/20 bg-white"
                            />
                        </div>
                    </div>
                    <div>
                        <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 block pl-1">Estatus Cartera Actual</label>
        ...
        if (nameLower.includes('seguimiento') || nameLower.includes('gestión') || nameLower.includes('visita')) {
            return (
    ...
    return (
        <div className="group border-b border-slate-50 hover:bg-slate-50/30 p-4 transition-all">
            <div className="flex items-center gap-6">
                ...
                <div className="w-32">
                    {/* 🚀 AÑADIDO: Ocultar selector de horas si es compromiso */}
                    {nameLower.includes('compromiso') ? (
                        <div className="input-cell !py-2 font-black text-[9px] text-amber-600 bg-amber-50 border-amber-100 flex items-center justify-center tracking-widest uppercase">
                            Todo el día
                        </div>
                    ) : (
                        <select
                ...
                {!isClientOnlySegment && (
                    <div className="w-48">
                        <select value={v.classification} onChange={e => updateVisit(segmentName, idx, 'classification', e.target.value)} className="input-cell !py-2 uppercase font-black tracking-widest text-[9px]">
                            ...
                            {!nameLower.includes('promoción') && !nameLower.includes('promocion') && <option>Cliente</option>}
                        </select>
                    </div>
                )}
                ...
                {!isClientOnlySegment && (
                    <div className="flex-1 min-w-0">
                        {nameLower.includes('evaluación') || nameLower.includes('integración') ? (
                            <select
                                value={v.activity}
...
const SegmentSection = ({ title, visits, segmentName }) => {
    const { currentAgenda, updateVisit, addRow, removeRow, clientsDatabase } = useAgenda();
    const isClientOnlySegment = segmentName.toLowerCase().includes('seguimiento') || segmentName.toLowerCase().includes('gestión') || segmentName.toLowerCase().includes('visita') || segmentName.toLowerCase().includes('compromiso');

    const safeVisits = visits || [];

    const isTimeAvailable = (time, currentVisitId) => {
        const allVisits = Object.values(currentAgenda.segments || {})
            .flatMap(vArr => (vArr || []).filter(v => v.time && v.id !== currentVisitId));
        ...
        for (const v of allVisits) {
            ...
            const isOnePromo = segmentName.toLowerCase().includes('promoción') || segmentName.toLowerCase().includes('promocion');
            const promoSegments = Object.entries(currentAgenda.segments || {})
                .filter(([name]) => name.toLowerCase().includes('promoción') || name.toLowerCase().includes('promocion'))
                .flatMap(([_, arr]) => arr || []);
                
            const involvesPromo = isOnePromo || promoSegments.some(p => p.id === v.id);

            if (involvesPromo && Math.abs(targetMin - vMin) < 30) return false;
        }
        return true;
    };
                        ...
                        <div key={v.id} className="glass-panel p-6 bg-white border border-slate-100">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {segmentName.toLowerCase().includes('compromiso') ? (
                                        <div className="border-none rounded px-3 py-1.5 text-[11px] font-black uppercase tracking-widest outline-none bg-amber-50 text-amber-600">Todo el día</div>
                                    ) : (
                                        <select
                                            value={v.time}
                                            onChange={(e) => updateVisit(segmentName, idx, 'time', e.target.value)}
...
const PlaneacionOperativo = () => {
    const { currentAgenda, sendForAuthorization, resetAgenda } = useAgenda();
    const { session } = useAuth();
    
    const [bloquesConfig, setBloquesConfig] = useState([]);
    const [isLoadingBloques, setIsLoadingBloques] = useState(true);

    useEffect(() => {
        const fetchBloques = async () => {
            if (!session?.idArea || !session?.puestoId) {
            ...
            try {
                ...
                const response = await axios.get(
                    `${API_COBRANZA}/bloques/area/${session.idArea}/puesto/${session.puestoId}`,
                    { withCredentials: true }
                );
                if (response.data.codigo === 'OK' && response.data.contenido) {
                    setBloquesConfig(response.data.contenido);
                }
            } catch (error) {
        ...
        <div className="max-w-[1400px] mx-auto pb-40 px-4 md:px-8">
            ...
            {isLoadingBloques ? (
                <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest">Cargando bloques de operación...</p>
                </div>
            ) : (
                bloquesConfig.map((bloque) => {
                    const safeVisits = (currentAgenda.segments && currentAgenda.segments[bloque.nombre]) ? currentAgenda.segments[bloque.nombre] : [];
                    return (
                        <SegmentSection 
                            key={bloque.idBloque} 
                            title={bloque.nombre.toUpperCase()} 
                            visits={safeVisits} 
                            segmentName={bloque.nombre} 
                        />
                    );
                })
            )}
            ...
            <footer className="fixed bottom-0 left-0 right-0 md:relative md:mt-20 z-40">
                <div className="bg-white/90 backdrop-blur-2xl border-t border-slate-200 p-6 md:p-12 md:bg-slate-900 md:text-white md:rounded-[40px] md:border-none shadow-[0_-20px_50px_rgba(0,0,0,0.1)] md:shadow-2xl flex flex-col md:flex-row justify-between items-center gap-6">
                    ...
                    <div className="flex flex-col md:items-end gap-3 w-full md:w-auto">
                        ...
                        <button onClick={() => sendForAuthorization(bloquesConfig)} className="w-full md:w-auto bg-primary md:bg-white md:text-primary text-white px-20 py-6 rounded-[24px] text-xs font-black uppercase tracking-[0.3em] shadow-2xl hover:scale-105 active:scale-95 transition-all">
                            Certificar Jornada
                        </button>
                    </div>
                </div>
